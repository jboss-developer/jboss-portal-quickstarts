...
/* Output a java code which can be pasted in a quickstart */
StringBuilder propBuilder = new StringBuilder("\n    /* External URLs generated by mvn install -P generate-readmes */\n");
...
keys.addAll(topProperties.keySet());
for (String key : keys) {
	if (key instanceof String && (key.endsWith(".url") || key.startsWith("compatibility."))) {
		boolean isUrl = key.endsWith(".url");
		if (isUrl || key.startsWith("compatibility.")) {
			String varName = key.replace('.', '_');
			propBuilder.append("    String ").append(varName).append(" = \"").append(topProperties.get(key)).append("\";\n");
